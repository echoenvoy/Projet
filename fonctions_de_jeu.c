...
int checkAndHandleMoulin(int iplace, int *itake, char adversarySymbol, int *adversaryPawns) {
    if (moulin(iplace)) {
        ...
        if (hasNonMoulinPawns) {
            printf("Vous avez formé un moulin ! Choisissez un pion adverse à capturer (ne faisant pas partie d'un moulin) : ");
            scanf("%d", itake);

            // Vérifier que le pion choisi est valide et ne fait pas partie d'un moulin
            ...
            // Vérifier que le pion choisi appartient à l'adversaire
            while (!(board[*itake] == adversarySymbol)) {
                printf("Position invalide. Veuillez choisir un pion appartenant à l'adversaire : ");
                scanf("%d", itake);
            }
        }

        // Capturer le pion et mettre à jour les pions restants
        board[*itake] = 'O';
        (*adversaryPawns)--;
        return 1; // Moulin validé et pion capturé
    }
...
void tourDePlacement(char joueurSymbol, char adversarySymbol, int joueurNum, int *adversaryPawns) {
    int iplace, itake;
    printf("Tour du joueur %d (%c). Choisissez une position pour placer votre pion (0 à 23) :\n", joueurNum, joueurSymbol);
    scanf("%d", &iplace);

    // Vérifier que la position est valide
    while (!isValid(iplace)) {
        printf("Position invalide. Veuillez choisir une position libre (0 à 23) :\n");
        scanf("%d", &iplace);
    }
    ...
    if (checkAndHandleMoulin(iplace, &itake, adversarySymbol, adversaryPawns)) {
        printf("Le joueur %d a capturé un pion adverse !\n", joueurNum);
    }
}
...
int is_win(char sj1, char sj2, int nbrspions1, int nbrspions2) {
    if (nbrspions1 <= 2 || isBlocked(sj1)) {//vérifie si un joueur ne peut plus bouger ou si si l'un des joueurs n’a que deux pions
