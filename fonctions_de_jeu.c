...
void TourDeMvt(char joueurSymbol, char adversarySymbol, int joueurNum, int *adversaryPawns) {
    ...
    do {
        ...
        if (board[ideplace] != joueurSymbol || !isValidToMove(ideplace, valides)) {
            printf("Ce pion ne peut pas être déplacé. Essayez un autre.\n");
            continue;
        }

    }  while (board[ideplace] != joueurSymbol || !isValidToMove(ideplace, valides));
    ...
    do {
        ...
        if (!isInArray(iaplace, valides, 4)) {
...
int isValidToMove(int ideplace,int* L){
   ...
   for (int j = 0; j < 4 && adjacences[ideplace][j] != -1; j++) {
         ...
         if (board[voisin] == 'O') { // Vérifier que la case est vide
            L[j] = voisin; // Ajouter à la liste des déplacements possibles
            a=1; 
            }
         }
   return a;// Retourne 1 si au moins un mouvement est possible, sinon 0
}
...
int is_win() {
    if (nbrspions1 <= 2 || isBlocked(sj1)) {//verifie si un joueur ne peut plus bouger ou si l'un des joueurs n’a que deux pions
        printf(GREEN"Le deuxieme joueur (%c) a gagne !\n"RESET,sj2);
        return 1; // Joueur 1 perd
    }
    if (nbrspions2 <= 2 || isBlocked(sj2)) {//verifie si un joueur ne peut plus bouger ou si l'un des joueurs n’a que deux pions
